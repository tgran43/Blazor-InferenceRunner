...
namespace Blazor_web.OnnxRuntime.utils
{
    public static class ImageHelper
    {
        ...
        /// <returns></returns>
        public static List<DenseTensor<float>> GetImageTensorFromPath(Image<Rgb24> UploadedImage, int imgWidth = 64, int imgHeight = 64)
        {
            // Read image
            //using Image<Rgb24> image = Image.Load<Rgb24>(imageFilePath);
            using Image<Rgb24> image = UploadedImage;
            // Resize image
            image.Mutate(x =>
            {
            ...
            var inputDimensions = new int[] { 1, 3, imgWidth, imgHeight };
